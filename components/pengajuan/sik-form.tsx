...
interface SIKFormProps {
  form: UseFormReturn<z.infer<typeof sikSchema>>;
  onSubmit: (values: z.infer<typeof sikSchema>) => void;
  isPending: boolean;
  penduduk: Penduduk;
}

export default function SIKForm({
  form,
  onSubmit,
  isPending,
  penduduk,
        ...
        <div className='grid gap-4 md:grid-cols-[1fr_0.7fr]'>
          ...
          <div className='grid auto-rows-max items-start gap-4'>
            ...
            <Card>
              ...
              <CardContent className='grid gap-4'>
                <div className='grid gap-3'>
                  <FormFieldInput<typeof sikSchema>
                    form={form}
                    name='jenisKeramaian'
                    label='Jenis Hiburan'
                    placeholder='Jenis Hiburan'
                    maxLength={255}
                    description='Tulis jenis atau nama hiburan yang akan dilaksanakan'
                  />
                </div>
                <div className='grid grid-cols-2 gap-3'>
                  <FormFieldInput<typeof sikSchema>
                    form={form}
                    type='date'
                    name='tanggalMulaiKeramaian'
                    label='Tanggal Mulai'
                    description='Tanggal mulai keramaian'
                  />
                  <FormFieldInput<typeof sikSchema>
                    form={form}
                    type='date'
                    name='tanggalSelesaiKeramaian'
                    label='Tanggal Selesai'
                    description='Tanggal selesai keramaian'
                  />
                </div>
                <div className='grid gap-3'>
                  <FormFieldTextarea<typeof sikSchema>
                    form={form}
                    name='keperluan'
                    label='Keperluan'
                    placeholder='Keperluan'
                    maxLength={255}
                    description='Keperluan membuat surat izin keramaian'
                  />
                </div>
              </CardContent>
            </Card>
